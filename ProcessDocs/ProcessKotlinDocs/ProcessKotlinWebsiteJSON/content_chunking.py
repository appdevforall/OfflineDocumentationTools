...
def served_fragment_paths(conn, base_path: str, base_length: int = None) -> list:
    ...
    to decide there is a chain at all, then rule 2 to stop at the first
    fragment shorter than CHUNK_SIZE.
    ...
    if not is_chunked_base({base_path: base_length}, base_path):
        return []
    served = []
    for _n, path, length in _discover(conn, base_path):
        served.append(path)
...
def split_into_chunks(data: bytes) -> list:
    """`data` split the way the server expects to read it back: the base
    blob first, then one entry per continuation row."""
